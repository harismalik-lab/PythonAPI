...
from delivery_cashless_api.v01.order_accepted.validation import order_accepted_parser
...
class OrderAccepted(BasePostResource):
    is_delivery_cashless = True
    required_token = True
    request_parser = order_accepted_parser
    response = {}
    logger_info = {
        'filename': '{log_path}{file_path}'.format(
            log_path=DELIVERY_LOGS_PATH,
            file_path='order_accepted/order_accepted.log',
        ),
        'name': 'order_accepted'
    }
    logger = None
    ...
    def generating_final_response(self):
        ...
        self.order_status = self.order_status_repo.get_filter_status(
            filters={'short_title': self.order_status_repo.Accepted}
        )
        self.merchant_order_changes = dict()
        self.merchant_order_changes['order_status_id'] = self.order_status.get('id')
        self.merchant_order_changes['delivery_time'] = self.delivery_time
